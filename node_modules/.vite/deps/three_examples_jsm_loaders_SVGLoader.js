import { Bs as Vector2, Eo as SRGBColorSpace, No as ShapePath, Po as ShapeUtils, U as Box2, Vs as Vector3, Wr as Loader, Y as BufferGeometry, jo as Shape, na as Path, ni as Matrix3, xn as Float32BufferAttribute, yn as FileLoader } from "./three.module-tsaMx_7e.js";
//#region node_modules/.pnpm/three@0.184.0/node_modules/three/examples/jsm/loaders/SVGLoader.js
	...
	*/
	parse(text) {
		...
		function parseNode(node, style) {
			...
			if (path) {
				if (style.fill !== void 0 && style.fill !== "none") path.color.setStyle(style.fill, COLOR_SPACE_SVG);
	...
	*/
	static createShapes(shapePath) {
		...
		function findEdgeIntersection(a0, a1, b0, b1) {
			...
			const t1 = nom1 / denom;
			const t2 = nom2 / denom;
			if (denom === 0 && nom1 !== 0 || t1 <= 0 || t1 >= 1 || t2 < 0 || t2 > 1) return null;
				...
				for (let i = 0; i < 2; i++) {
					...
					} else if (classifyResult.loc == IntersectionLocationType.BETWEEN) return {
		...
		function getScanlineIntersections(scanline, boundingBox, paths) {
			...
			paths.forEach((path) => {
				if (path.boundingBox.containsPoint(center)) getIntersections(scanline, path.points).forEach((p) => {
		...
		function isHoleTo(simplePath, allPaths, scanlineMinX, scanlineMaxX, _fillRule) {
			if (_fillRule === null || _fillRule === void 0 || _fillRule === "") _fillRule = "nonzero";
			const centerBoundingBox = new Vector2();
			simplePath.boundingBox.getCenter(centerBoundingBox);
			const scanlineIntersections = getScanlineIntersections([new Vector2(scanlineMinX, centerBoundingBox.y), new Vector2(scanlineMaxX, centerBoundingBox.y)], simplePath.boundingBox, allPaths);
		...
		for (let identifier = 0; identifier < simplePaths.length; identifier++) simplePaths[identifier].identifier = identifier;
		...
		simplePaths.forEach((p) => {
			if (!isAHole[p.identifier].isHole) {
				const shape = new Shape();
				shape.curves = p.curves;
				isAHole.filter((h) => h.isHole && h.for === p.identifier).forEach((h) => {
